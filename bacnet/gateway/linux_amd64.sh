...
cd cmake
rm -rf *
cmake -DCMAKE_C_COMPILER=$G_CC -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=x64 -DCMAKE_SYSTEM_VERSION=1 -DBUILD_SHARED_LIBS=Off -DCMAKE_INSTALL_PREFIX=$OUTPUTDIR  -DENABLE_CJSON_TEST=FALSE -DCMAKE_C_FLAGS=-m64 ../cJSON-1.5.9/
cmake --build .
make install
fi
...
make CC=$G_CC RANLIB=$G_RANLIB LD=$G_LD MAKEDEPPROG=$G_CC PROCESSOR=x64 
...
cmake -DCMAKE_C_COMPILER=$G_CC -DCMAKE_CXX_COMPILER=$G_CPP -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=x64 -DCMAKE_SYSTEM_VERSION=1 -DPAHO_WITH_SSL=TRUE -DPAHO_BUILD_STATIC=TRUE -DOPENSSL_SEARCH_PATH=$OUTPUTDIR -DOPENSSLCRYPTO_LIB=$OUTPUTDIR/lib/libcrypto.a -DOPENSSL_LIB=$OUTPUTDIR/lib/libssl.a  -DOPENSSL_INCLUDE_DIR=$OUTPUTDIR/include -DPAHO_BUILD_SAMPLES=FALSE -DCMAKE_C_FLAGS=-m64  ../paho.mqtt.c-1.2.0/
...
cd $BASEDIR
make clean
make IOT_LIB_DIR=$OUTPUTDIR/lib IOT_INC_DIR=$OUTPUTDIR/include CC=$G_CC AR=$G_AR BUILD=release ARCH=amd64
cp bdBacnetGateway ../bin/linux_amd64/bdBacnetGateway
echo "======================================="
echo "SUCCESS, executable is located at ../bin/linux_amd64/bdBacnetGateway"
